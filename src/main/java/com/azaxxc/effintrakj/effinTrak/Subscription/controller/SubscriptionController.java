...
public class SubscriptionController {
    ...
    public ResponseEntity<Object> getSubscriptionsByUserId(@PathVariable Long userId, Authentication authentication) {
        Long effectiveUserId = authenticatedUserResolver.resolveRequestedUserId(authentication, userId);
        List<SubscriptionResponseDTO> subscriptions = subscriptionService.getSubscriptionsByUserId(effectiveUserId);
        return globalResponseService.success(subscriptions, "Fetched subscriptions for user");
    }

    @GetMapping("/{id}")
    public ResponseEntity<Object> getSubscriptionById(@PathVariable Long id, Authentication authentication) {
        try {
            Long authenticatedUserId = authenticatedUserResolver.resolveUserId(authentication);
            SubscriptionResponseDTO subscription = authenticatedUserId == null
                    ? subscriptionService.getSubscriptionResponseById(id)
                    : subscriptionService.getSubscriptionResponseByIdForUser(authenticatedUserId, id);
    ...
    public ResponseEntity<Object> updateSubscription(@PathVariable Long id, @RequestBody UpdateSubscriptionRequestDTO dto,
            Authentication authentication) {
        Long authenticatedUserId = authenticatedUserResolver.resolveUserId(authentication);
        SubscriptionResponseDTO updatedSubscription = authenticatedUserId == null
                ? subscriptionService.updateSubscription(id, dto)
                : subscriptionService.updateSubscriptionForUser(authenticatedUserId, id, dto);
    ...
    public ResponseEntity<Object> getUpcomingRenewals(
            ...
            Authentication authentication) {
        Long effectiveUserId = authenticatedUserResolver.resolveRequestedUserId(authentication, userId);
        List<SubscriptionResponseDTO> subscriptions = subscriptionService.getUpcomingRenewals(effectiveUserId, days);
        return globalResponseService.success(subscriptions, "Fetched upcoming subscription renewals");
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Object> deleteSubscription(@PathVariable Long id, Authentication authentication) {
        Long authenticatedUserId = authenticatedUserResolver.resolveUserId(authentication);
        if (authenticatedUserId == null) {
            subscriptionService.deleteSubscription(id);
        } else {
            subscriptionService.deleteSubscriptionForUser(authenticatedUserId, id);
        }
